...
- an unused application hostname, such as `chat.example.com`;
...
4. If using open registration, test it in a private browser window with a second address.
5. If using invite-only registration, confirm an uninvited identity does not become an active member.
