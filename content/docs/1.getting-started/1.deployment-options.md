...
The installer uses temporary Cloudflare OAuth access to provision the bindings, deploy the Worker, configure Cloudflare Access email-code login, optionally attach a custom hostname and workspace mail, apply migrations, and return the workspace URL. The installed Worker does not retain the Cloudflare API token.
...
| Cloudflare Access | Installer by default | You |
