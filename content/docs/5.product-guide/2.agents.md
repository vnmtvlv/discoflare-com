...
The Worker may become reachable before the Agent Computer image finishes provisioning. After a fresh deploy or upgrade, allow several minutes before treating the first failed Agent run as a persistent problem.
...
Each Agent has one stable Computer identity. Its filesystem is durable in the Agent Durable Object, while an isolated Container backend provides Linux command execution when needed. Large artifacts belong in R2; the Container itself can sleep without becoming the Agent's storage identity.
