import { CLOUDFLARE_AUTH_URL, CLOUDFLARE_UNINSTALL_SCOPES, oauthConfig, randomBase64Url, sha256Base64Url } from '../../../utils/cloudflare-oauth'
...
export default defineEventHandler(async (event) => {
  ...
  const returnTo = typeof query.returnTo === 'string' && query.returnTo.startsWith('/') && !query.returnTo.startsWith('//')
    ? query.returnTo
    : '/deploy'
  const config = oauthConfig(event, returnTo.startsWith('/uninstall') ? CLOUDFLARE_UNINSTALL_SCOPES : undefined)
